...
class GameEvaluation04(val game : Game, val self : Faction) extends GEvaluation {
    def eval(a : Action) : $[Evaluation] = {
        ...
        a.unwrap match {
            ...
            case StealFromAction(_, f) =>
                ...
                death && duel && players(f).money <= 8 |=> 100000 -> "d&d"
            ...
            case StealBlockWithAction(_, _, Ambassador) =>
                has(Ambassador) |=> 200 -> "block with ambassador"
            ...
            case KillWhomAction(_, f) =>
                ...
                has(Assassin).not && see(Assassin) == 3 |=> -1000000 -> "no more"
                death && duel |=> 100000 -> "d&d"
